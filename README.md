...
~~多容器任务相关脚本不支持2.10.9及其以上版本(拉取的仓库生成的对应文件夹无法同步)，待适配，这周六日进行适配~~

多容器任务相关脚本已修改适配2.10.10及其以上版本的文件同步，但有个缺陷，如果是新拉的仓库，得在被同步的青龙里创建对应的文件夹，这个bug暂时已请教面板作者，待回复
...
scripts_check_dependence.py，也就是二叉树修复依赖文件脚本，拉取后运行一次，可以查查自己有啥脚本依赖文件没装的，在配置文件中配置，可以修补缺失文件(jdCookie.js那种)，不建议开启内置的依赖更新，那个有魔改的只适用于对应的config--->https://github.com/spiritLHL/dependence_config/
...
~~多容器任务相关脚本不支持2.10.9及其以上版本(拉取的仓库文件无法同步)，待适配，这周六日进行适配~~

单容器查询网络链接适配所有版本，暂时没有2.10.9版本及其以上版本的仓库对应文件夹识别，待开发

修复单容器脚本无法识别是ql raw还是ql repo的bug
