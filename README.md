...
2.没计算好分发数量，导致容器分发溢出的，默认分发每个容器35个，只能少不能多，如果你的```主容器(青龙)变量总数```超过```35×分容器(青龙)数量```，那么分发会溢出报错

错误截图如下：

![](https://i.loli.net/2021/11/05/xzuwjvdQoXR25rK.png)

解决方法：在分发脚本286行中自行更改每个容器的环境变量数量，默认35修改为其他，或增加分容器数量使数据不溢出

3.分发的默认变量```不含关键字```，需要在环境变量管理中查询```关键字```，然后勾选全部禁用，再刷新页面，剩下启用的就是```不含关键字```的导致分发报错的变量，需要剔除才能进行分发

4.分容器(青龙)中自身含环境变量，后续增加导致那个容器(青龙)环境变量超过35个(默认配置)，分发前请清空分容器变量，避免分发的变量和原有变量出现混合，记得做好备份
