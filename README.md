...
**仅支持公网IP能访问的版本2.9.0以上的青龙，至多到2.10.12已适配**

多容器任务相关脚本已修改适配2.10.10及其以上版本的文件同步，但有个缺陷，如果是新拉的仓库，得在被同步的青龙里创建对应的文件夹，这个bug暂时已请教面板作者，待2.10.13版本修复
...
更新日志：
    1.分发脚本支持备份容器多个
    2.增加单容器脚本 二叉树监控脚本运行，多容器请配合自动启用禁用的多容器脚本使用
    3.优化环境同步脚本识别不同步JD_WSCK的值，不再新增该变量名对应的值
    4.优化环境依赖脚本访问Github频繁的问题，使用第三方网址替换
```
